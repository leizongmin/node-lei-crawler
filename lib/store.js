...
function SimpleStore (options) {
  ...
  var redis = exports._redis = new Redis({
    host: options.host,
    port: options.port,
    db: options.port
  });
...
SimpleStore.prototype.sAdd = function (name, values, callback) {
  this._redis.sadd(this._getKey(name), this._getValues(values), callback);
};

SimpleStore.prototype.sRemove = function (name, values, callback) {
  this._redis.srem(this._getKey(name), this._getValues(values), callback);
};

SimpleStore.prototype.sExists = function (name, value, callback) {
  this._redis.smember(this._getKey(name), value, function (err, ret) {
    callback(err, ret > 0);
  });
};
...
SimpleStore.prototype.get = function (name, value, callback) {
...
SimpleStore.prototype.setCache = function (name, value, ttl, callback) {
  this._redis.set(this._getKey(name), ttl, value, callback);
};
