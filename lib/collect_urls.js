...
URLCollector.prototype._collectPage = function (depth, url, callback) {
  ...
  self._options.store.sExists('collected', value, function (err, yes) {
    ...
    self._loadURL(url, function (err, list) {
      ...
      async.series([
        function (next) {
          ...
          self._options.store.sAdd('collected', value, next);
        ...
        function (next) {
          ...
          self._options.store.sAdd('wait', depthList, next);
        ...
        function (next) {
          ...
          self._options.store.sAdd('result', resultList, next);
...
URLCollector.prototype.start = function (url, callback) {
  ...
  async.series([
    function (next) {
      ...
      self._options.store.sRemove('collected', [value], ignoreError(next));
    ...
    function (next) {
      ...
      function nextPage (err) {
        ...
        self._options.store.sGet('wait', function (err, data) {
          if (err) callback(err);
